...
\subsubsection*{What are Shadows?}
A shadow is a region of reduced illumination created when an opaque object blocks light from a source. Geometrically, shadows result from the intersection of light rays with surfaces, creating visibility constraints that encode rich 3D information.
\begin{definition}[Shadow Formation] ~\\
Given a light source $\mathbf{L}$ and surface $\mathbb{S}$, a point $\mathbf{P}$ is in shadow if there exists a surface point $\mathbf{Q} \in \mathbb{S}$ such that $\mathbf{Q}$ lies on the line segment $\overline{\mathbf{L}\mathbf{P}}$.
\end{definition}
